...
** **DO NOT USE ANY OTHER CONFIGS** ** unless it is stated in the [Updates](#Updates) section and reflected in the configuration examples listed under [Quick References](#Quick-References). Remember, we'll be grading based on the current configurations, so any errors will be marked down based on those.
...
Now that ESLint is installed globally, we can initialize a configuration file for eslint that will be named `.eslintrc.json`. ESLint has an "--init" option that walks you through making initial configurations to your `.eslintrc.json`, which can be executed by entering in the following command into your command line (make sure that you are still at the root of your project directory):
...
The most current configuration of our class's `.prettierrc` file can be found under [Quick References](#Quick-References).
